...
<form id="<?=$arParams["FORM_ID"]?>" name="<?=$arParams["FORM_ID"]?>" action="<?=POST_FORM_ACTION_URI?>" method="post" enctype="multipart/form-data">
	...
	<?if (is_array($arResult["PROPERTY_LIST"]) && !empty($arResult["PROPERTY_LIST"])) :
		foreach ($arResult["PROPERTY_LIST"] as $propertyID) :?>
			...
			if (in_array($propertyID,$arParams["PROPERTY_CODES_PHONE"]))
				$validation .= ' data-phone="yes_'.$arParams["FORM_ID"].'"';
	...
	<p>Нажимая на кнопку "<?=$btmName?>", Вы соглашаетесь на обработку персональных данных в соответствии с <a href="/privacy_policy/" target="_blank">политикой конфеденциальности</a></p>
...
<script>
	...
	$("[data-phone='yes_<?=$arParams["FORM_ID"]?>']").mask("+7 (999) 999-99-99");
</script>
<?endif?>