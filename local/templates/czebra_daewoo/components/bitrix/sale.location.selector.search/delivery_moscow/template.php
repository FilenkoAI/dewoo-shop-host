...
<?else:?>
	...
	<script>
		...
		<?endif?>
			...
			new BX.Sale.component.location.selector.searchMoscow(<?=CUtil::PhpToJSObject(array(
...
<?endif?>
<script>
    document.querySelectorAll('#sls-<?=$arResult['RANDOM_TAG']?> .bx-ui-sls-fake')[0].onblur = function() {
        if($('#sls-<?=$arResult['RANDOM_TAG']?> .bx-ui-sls-route').val() == ''){
            $('#sls-<?=$arResult['RANDOM_TAG']?> .bx-ui-sls-fake').val('');
        }
    };
</script>
