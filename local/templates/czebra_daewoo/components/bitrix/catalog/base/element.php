...
?>
    ...
    <div class="wrapp-card">
        <div class="container">
            ...
            // Инструкция
            $manualID = '';
            $arSelect = array("ID", "NAME", "PROPERTY_INSTRUCTION", "PROPERTY_ACCESSORIES");
            $arFilter = array("IBLOCK_ID" => 29, "ACTIVE_DATE" => "Y", "ACTIVE" => "Y", "PROPERTY_CML_ELEM.ID" => $elementId);
            $res = CIBlockElement::GetList(array(), $arFilter, false, array("nPageSize" => 49), $arSelect);
            while($arFields = $res->Fetch()) {
                $manualID = $arFields['PROPERTY_INSTRUCTION_VALUE'];
                if ($arFields['PROPERTY_ACCESSORIES_VALUE']){
                    $recommendedProducts[] = $arFields['PROPERTY_ACCESSORIES_VALUE'];
                }
            }


            $ob = CFile::GetById($manualID);
            $manual = [];

            if($res = $ob->Fetch()) {
                $manual["LINK"] = '/upload/' . $res['SUBDIR'] . '/' . $res['FILE_NAME'];
                $manual["SIZE"] = formatBytes($res["FILE_SIZE"]);
                $manual["NAME"] = $res['NAME'];
                $arResult['DOCUMENTS']['MANUAL'] = $manual;
            }
            ...
            $certificateID = '';
            $arSelect = array("ID", "NAME", "PROPERTY_CERTIFICATE");
            $arFilter = array("IBLOCK_ID" => 34, "ACTIVE_DATE" => "Y", "ACTIVE" => "Y", "PROPERTY_PRODUCTS.ID" => $elementId);
            $res = CIBlockElement::GetList(array(), $arFilter, false, array("nPageSize" => 49), $arSelect);
            if($ob = $res->GetNextElement()) {
                $arFields = $ob->GetFields();
                $certificateID = $arFields['PROPERTY_CERTIFICATE_VALUE'];
            }
            $ob = CFile::GetById($certificateID);
            $certificate = [];
            if($res = $ob->Fetch()) {
                $certificate["LINK"] = '/upload/' . $res['SUBDIR'] . '/' . $res['FILE_NAME'];
                $certificate["SIZE"] = formatBytes($res["FILE_SIZE"]);
                $certificate["NAME"] = $res['NAME'];
                $arResult['DOCUMENTS']['CERTIFICATE'] = $certificate;
            }
            ?>
            <? //мобильная версия?>
            <? require(realpath(dirname(__FILE__)) . '/mobile.php') ?>
            <? //--мобильная версия?>
            <? // Табы?>
            <div class="wrapp-tabs-card">
                ...
                <div class="tab-content">
                    <? require(realpath(dirname(__FILE__)) . '/tabs/characteristics-tabs.php') ?>
                    <? require(realpath(dirname(__FILE__)) . '/tabs/service-support-tabs.php') ?>
                    <? require(realpath(dirname(__FILE__)) . '/tabs/getting-product-tabs.php') ?>
                    <? require(realpath(dirname(__FILE__)) . '/tabs/reviews-tabs.php') ?>
                    <? require(realpath(dirname(__FILE__)) . '/tabs/garanty-xxl-tabs.php') ?>
                </div>
            </div>
            <?
            ...
            $arrFilterRecommended = [
                "ID" => $recommendedProducts
            ];
            ?>
            <? //Рекомендуем с этой моделью?>
...
?>
<? if($request->get("reviews") == 'Y'): ?>
    <script>
        $('[href="#reviews-tabs"]').tab('show');
        const el = document.querySelector('[href="#reviews-tabs"]');
        el.scrollIntoView();
    </script>
<? elseif($request->get("reviews_mobile") == 'Y'): ?>
    <script>
        const el = document.querySelector('[href="#open-reviews-mobile"]');
        el.scrollIntoView();
    </script>
<? endif; ?>
<?if($request->get("pickup") == 'Y'):?>
    <script>
        $('[href="#getting-product-tabs"]').tab('show');
        $('[href="#pickup"]').tab('show');
        const el = document.querySelector('[href="#reviews-tabs"]');
        el.scrollIntoView();
    </script>
<?endif?>
<?if($request->get("delivery") == 'Y'):?>
    <script>
        $('[href="#getting-product-tabs"]').tab('show');
        $('[href="#delivery-company"]').tab('show');
        const el = document.querySelector('[href="#reviews-tabs"]');
        el.scrollIntoView();
    </script>
<?endif?>