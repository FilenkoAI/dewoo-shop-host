...
BX.Iblock.Catalog.CompareClass = (function()
{
	...
	CompareClass.prototype.delete = function(url, id)
	{
		BX.showWait(BX(this.wrapObjId));
		BX.ajax.post(
			url,
		...
		$('[data-slide-del="id_' + id + '"]').remove();
		if($('.wrapp-list-compare .slider-compare .slick-list .slick-track > div').length <= 1){
