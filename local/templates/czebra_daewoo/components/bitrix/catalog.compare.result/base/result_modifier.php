...
foreach($arResult['ITEMS'] as &$arItem){
    if($arItem["PRICES"][$arItem["CATALOG_GROUP_NAME_1"]]["PRINT_VALUE"]) {
        $oldPrintPrice = $arItem["PRICES"][$arItem["CATALOG_GROUP_NAME_1"]]["PRINT_VALUE"];
        $oldPrice = $arItem["PRICES"][$arItem["CATALOG_GROUP_NAME_1"]]["VALUE"];
    } elseif($arItem["PRICES"][$arItem["CATALOG_GROUP_NAME_2"]]["DISCOUNT_DIFF"] > 0) {
        $oldPrintPrice = $arItem["PRICES"][$arItem["CATALOG_GROUP_NAME_2"]]["PRINT_VALUE"];
        $oldPrice = $arItem["PRICES"][$arItem["CATALOG_GROUP_NAME_2"]]["VALUE"];
    }

    $arItem['CZ_PROPS']['PRICES'] = [
        'OLD_PRICE' => [
            'DISPLAY_VALUE' => str_replace("руб.","<span class='rubl'>i</span>",$oldPrintPrice),
            'VALUE' => $oldPrice
        ],
        'PRICE' => [
            'DISPLAY_VALUE' => str_replace("руб.","<span class='rubl'>i</span>",$arItem["PRICES"][$arItem["CATALOG_GROUP_NAME_2"]]["PRINT_DISCOUNT_VALUE"]),
            'VALUE' => $arItem["PRICES"][$arItem["CATALOG_GROUP_NAME_2"]]["DISCOUNT_VALUE"]
        ],
    ];
    ...
    $arItem['CZ_PROPS']['COMPARED'] = array_key_exists($arItem['ID'], $itemsInCompareList) ? 'Y' : 'N';
}
